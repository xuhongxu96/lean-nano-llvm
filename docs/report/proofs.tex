...
right-hand side returns some value, the source program on the left-hand side
can already return that same value as well.
...
Here \texttt{llvm-1-definition} denotes a quasiquoter for definitions with one
...
identity on the same input value. The two occurrences of \vundef on the left
may resolve independently, while the one occurrence on the right resolves to a
single concrete value. Since the left-hand side is the source and the
right-hand side is the target, the proof uses the refinement definition's
existential freedom on the left-hand-side supply. Given a supply for the
    ...
    | cons x xs => ~\label{line:undef-proof-cons-supply}~
      have hmulRun : ... =
          Except.ok (RegisterValue.bv w
            (.value ((BitVec.ofNat w x) * (2 : BitVec w)))) := by ~\label{line:undef-proof-hmul}~
        simp [simp_llvm]
        rfl
      refine ⟨[x, x], ?_⟩ ~\label{line:undef-proof-witness}~
      have haddRun : ... =
          Except.ok (RegisterValue.bv w
