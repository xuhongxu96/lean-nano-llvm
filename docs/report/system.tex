...
Expressions denote integer values of a chosen width. Integer literals and
...
already be producible by the source definition \texttt{x}, assuming both
definitions are well formed, have compatible signatures, and are run on
...
produced by \texttt{y} must already be reproducible by \texttt{x}, possibly
under a different \vundef supply. This asymmetry is what allows the target to
be more defined than the source without introducing any genuinely new
observable behavior.
